...
import net.ent.etrs.repaspatient.model.daos.RepasMemDao;
...
public class FacadeMetierImpl implements FacadeMetier{
	...
	PatientMemDao pdao = DaoFactory.fabriquePatientDao();
	RepasMemDao rdao = DaoFactory.fabriqueRepasDao();
	...
	public List<Patient> listerPatients() {
		return Collections.unmodifiableList(pdao.readAll());
	}
	...
	public List<Repas> listerRepas() {
		return Collections.unmodifiableList(rdao.readAll());
	}
	 ...
	 */
	@Override
	public void mettreAJourPatient(final Patient patient) throws BusinessException, DaoException {
		if(Objects.isNull(patient)){
			throw new DaoException(ConstantesMetier.FACADE_METIER_PATIENT_NULL);
		}
		try {
			pdao.update(patient);
		} catch (DaoException e) {
...
//            miney.ajouterRegimeAlimentaire(RegimeAlimentaire.VEGAN);
			...
			this.pdao.create(miney);
			this.pdao.create(haddock);
			this.pdao.create(tintin);
			this.pdao.create(tournesol);
			this.pdao.create(tombR);
			this.pdao.create(tombP);
		} catch (PatientConstructionException | RepasConstructionException | DaoException |
				 RepasException | PatientException e) {
			throw new BusinessException(e.getMessage(), e);
		}
